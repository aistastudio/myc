...
test(
  ...
  async () => {
    ...
    const m = await measureAsync(
      ...
      {
        warmup: 8,
        // 100, а не 25: бюджет по p99, а p99 по nearest-rank при n < 100 —
        // максимум прогона; при 25 замерах один сосед по процессору в двух
        // прогонах из трёх решал за весь замер. Цена: ~10 с здесь, ~20 с на
        // раннере (соперник — все 16 источников).
        iters: 100,
