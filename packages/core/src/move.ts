 ...
 *     весь связный кусок.
 *
 * Рёбра ОСТАЛЬНЫХ типов (relates, mentions, touches…) границу пересекать
 ...
 *
 * `parent` ПЕРЕСТАЛ быть одним из них с миграции 10 (memory-atcm254ry6c7):
 * блокеры наследуются вниз по дереву счётчиком `nodes.anc_blockers`, который
 * ведут те же триггеры и в той же одной базе. Поэтому узел, уезжающий
 * из-под заблокированного эпика, — это ровно случай (2), и он тоже отказ
 * (`cross_boundary_parent`). Проверка живёт не здесь, а в planMove
 * (packages/store-sqlite/src/move.ts): она читает материализованный счётчик,
 * а этот модуль чистый и о базе ничего не знает.
 *
