...
test("recall на 100k из чужого репозитория укладывается в бюджет И1", async () => {
  ...
  const m = await measureAsync(`recall @${N} узлов, охват чужого репозитория`, () => once(), {
    warmup: 3,
    // 100, а не 12: бюджет — по p99, а p99 по nearest-rank при n < 100 — это
    // максимум прогона, то есть один сосед по процессору (при 12 замерах —
    // любой из них). 100 замеров переживают один выброс на прогон, медиана
    // трёх прогонов — выбросы в одном из них. Цена: ~4 с на этом стенде.
    iters: 100,
