...
/**
 * Во сколько раз чтение файлов обязано опережать `git rev-parse`. Измерено
 * ×139.89. Порог 20 — с запасом почти на порядок: он не различает 100× и
 * 400×, но отделяет два read() от запуска процесса при любой загрузке.
 */
const MIN_AHEAD_OF_GIT = 20;
