...
/**
 * СПАН РАЗНЫЙ НА КАЖДОЙ ЗАПИСИ, и это не украшение замера. Второй факт на ТОТ
 * ЖЕ спан того же файла не заводит нового якоря, а присоединяется к уже
 * стоящему (общий якорь, memory-s32xpa09ytpb): ни узла, ни строки якоря, ни
 * работы фона. Мерить, не разводя спаны, значило бы мерить присоединение, а
 * не привязку. До общего якоря это было ещё хуже: второй
 * падал на `ux_nodes_content`, и на первом прогоне соперник вышел «быстрее»
 * здорового, потому что обе половины падали на уникальном индексе за 0.2 мс.
 */
