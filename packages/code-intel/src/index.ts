 ...
 * ТЕКСТОВАЯ, без AST, и она обязательна; graft — вторая реализация того же
 * интерфейса, и она опциональна. Умолчание `builtin` ВЕЗДЕ: определять
 * реализацию по окружению значит вернуть «у меня работает иначе», ради ухода
 * от чего всё и затевалось.
 *
 * Здесь — интерфейс (§6.1) и выбор реализации (`./select.ts`, §6.2). Сами
 * реализации живут отдельно: `BuiltinCodeIntel` поверх `./defs.ts` (T3) и
...
export type CodeIntelCapability =
  ...
  | "anchor"
  /** символ → path:span. builtin: языки L1; graft: девять языков. */
  | "symbol"
  /** число вхождений. builtin: текст; graft: граф. Источник подписан всегда. */
  | "fan_in"
  /** рёбра вызовов — только graft. */
  | "callers"
  /** ранжированный поиск по коду — только graft. */
  | "search"
  /** карта репозитория — только graft. */
  | "map";
...
export interface SymbolHit {
  ...
  /** Что это по мнению реализации: builtin различает по регулярке, graft по AST. */
...
/** Строка ранжированной выдачи поиска по коду. Только graft. */
...
/** Кластер карты репозитория. Только graft. */
 ...
 *
 * Необязательные методы — ровно те возможности, которых у builtin нет.
 * Проверять их наличие полагается через `capabilities`, а не через
