 ...
 * СПАВНОМ ПРОЦЕССА и потому ловит всё, что делает машина рядом: на
 * неизменном бинаре три прогона по 25 спавнов подряд дали p50 24.07 / 23.97 /
 ...
 * то есть один сосед по CPU; p99 — просто максимум (теперь спавнов 100 × 3,
 * см. coldStartOp, но хвост спавна шумнее медианы и так). Сравнивать линию по
...
async function coldStartOp(): Promise<Timed> {
  ...
  //
  // 100 × 3, а не прежние 25 × 1: регрессия к линии и так идёт по p50 (см.
  // REGRESSION_METRIC), но БЮДЖЕТ И1 (p99 < 60 мс) проверяется по p99, а p99
  // по nearest-rank при n < 100 — максимум прогона. При 25 спавнах в одном
  // прогоне один медленный спавн (наблюдалось 53.98 мс при p50 25) решал за
  // весь бюджет на свободной откалиброванной машине и в строгом режиме. 100
  // замеров переживают выброс в прогоне, медиана трёх — выбросы в одном из
  // них. Цена: ~300 спавнов по ~20 мс плюс эталон той же длительности — ~13 с.
  const m = await measureAsync("cold_start", spawnOnce, { warmup: 3, iters: 100, trials: 3 });
  return { ...m.stats, jitter: m.jitter };
}
