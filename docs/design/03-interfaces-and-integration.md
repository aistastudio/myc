...
status, since, limit, offset — с потолком) и `GET /v1/ws/:ws/nodes/:id` (узел
и его рёбра). Конверт — тот же, что у CLI (§2.3).
...
**Записи через сервер пока нет, и это не забывчивость, а неразрешённая
развилка.** Правила слияния myc (оплог, HLC, часы полей, разбор двойников)
...
Чего сознательно нет: OIDC/SSO (задача прокси, как и в спеке), ролей и
областей, `--trust-proxy` с `X-Forwarded-User` (доверие заголовку — отдельная
опасность, и заводить её до появления самого прокси незачем).
