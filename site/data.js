...
window.MYC_DATA = {
    ...
    "groups": [
      {
        ...
        "items": [
          ...
          },
          {
            "since": "0.3.7",
            "cmd": "myc attempt list",
            "en": "A task's class for routing is taken from the files the attempt actually changed — against a git snapshot taken at its start, across nested repositories and worktrees — and attempt list shows the distribution.",
            "ru": "Класс задачи для роутинга берётся из файлов, которые попытка действительно изменила, — против git-снимка на её старте, по вложенным репозиториям и worktree; attempt list показывает распределение."
          }
        ]
      },
      {
      ...
      },
      {
        ...
        "items": [
          ...
          },
          {
            "since": "0.3.9",
            "cmd": "myc code symbol",
            "en": "fan_in — how often a symbol is mentioned — is counted in the background at the end of every index run and read ready-made; knowledge whose anchor is lost or stale ranks lower in search (lost ×0.2, stale ×0.5) and is marked.",
            "ru": "fan_in — сколько раз упомянут символ — считается в фоне в конце каждого прогона индекса и читается готовым; знание с потерянным или устаревшим якорем ниже в поиске (потерян ×0.2, устарел ×0.5) и помечено."
          }
        ]
      },
      {
      ...
      },
      {
        ...
        "items": [
          ...
          {
            "since": "0.3.11",
            "cmd": "myc wire --scope user --agents opencode",
            "en": "opencode in a worktree gets myc from its global config: one <code>mcp.myc</code> node in <code>opencode.json[c]</code>, comments and other entries kept byte for byte, and a plugin that stays silent where there is no workspace or the project wires opencode itself.",
            "ru": "opencode в worktree получает myc из своего глобального конфига: один узел <code>mcp.myc</code> в <code>opencode.json[c]</code>, комментарии и чужие записи — байт в байт, и плагин, который молчит там, где нет воркспейса или проект сам подключает opencode."
          }
        ]
      },
      {
        ...
        "items": [
          ...
          },
          {
            "since": "0.3.9",
            "cmd": "myc anchor add",
            "en": "An anchor is never bound to a secret-named file, a directory or a binary: refused before anything is written, so a .env's content never reaches a node.",
            "ru": "Якорь не ставится на секретный по имени файл, каталог или бинарный файл: отказ до записи узла, и содержимое .env не попадает в узел."
          }
        ]
      },
      {
        ...
        "items": [
          ...
          },
          {
            "since": "0.3.11",
            "cmd": "myc doctor",
            "en": "The package ships its own SQLite for macOS (3.53.4, arm64 and x86_64), and every command picks the library the same way before it opens the database — <code>MYC_SQLITE</code>, the bundled one, Homebrew. A SQLite too old to write to is refused with what to install, and <code>doctor</code> names the one in use.",
            "ru": "Пакет везёт свою SQLite для macOS (3.53.4, arm64 и x86_64), и каждая команда выбирает библиотеку одинаково, до открытия базы, — <code>MYC_SQLITE</code>, своя, Homebrew. Слишком старая SQLite, в которую нельзя писать, — отказ с тем, что поставить; <code>doctor</code> называет действующую."
          },
          {
            "since": "0.3.11",
            "cmd": "myc doctor --background",
            "en": "<code>doctor</code> warns when the anchor sweep or the code index has not run for two of its periods while work went on; a build checks that the new binary runs the background work before it replaces the old one.",
  ...
  "planned": {
    ...
    "epics": [
      {
        "id": "memory-5xravkn0anzk",
        "en": "The core works, this project and a second one run on it, and the milestone's acceptance is passed; what is open is two known costs.",
        "ru": "Ядро работает, на нём живут этот проект и второй, приёмка вехи пройдена; открыты две известные цены.",
        "items": [
          {
            "id": "memory-vt0rkpv0xwn2",
  ...
  "tests": {
    ...
    "assertions": 37508,
    "seconds": 378.2,
    "sources": "f9a81b691b5523d1",
    "date": "2026-09-26",
    "myc": "0.3.14"
  }
};
window.MYC_DATA.verified = { at: "2026-09-26T07:00:37.440Z", assertions: 77 };
